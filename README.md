...
**학습된 모델 12종의 실제 출력**이다 (기준일 2026-09-01). 전 고객 100만 명 중
고정 시드로 뽑은 **12,005명 표본** + 데모 고객 5명이다.

세분 예측 8종이 상품 가입 여부에 따라 채워지거나 비며, 화면의 「예측·신호
커버리지」 카드가 **"해당 상품 미가입"과 "미제공"을 구분해서** 보여준다.

확률 분포가 합성과 다르므로(학습된 모델의 최대 해지 확률이 0.686) 정책 임계값도
...
```bash
cd backend && POPULATION_SOURCE=prediction LLM_ENABLED=false \
