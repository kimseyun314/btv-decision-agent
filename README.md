...
화면③의 「AI 판단 근거」 문장은 **Gemini(gemini-3.6-flash)가 생성한 것**이다.
9,685명 전원에 대해 한 번 돌려 스냅샷에 굳혔고(`reason_source: llm` 100%),
그 뒤로는 LLM 을 호출하지 않는다 — 정적 파일을 읽을 뿐이다.

**LLM 이 결정을 만들지는 않는다.** 액션과 점수는 Policy Layer 가 정하고, LLM 은
그 결정의 근거를 문장으로 옮기기만 한다. Guardrail 이 (a) 결정된 액션 미인용
(b) 발동하지 않은 정책 인용을 잡아 규칙 기반 템플릿으로 되돌린다.
